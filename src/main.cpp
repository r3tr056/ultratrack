...
void print_usage() {
    ...
    std::cout << "Options:\n";
    std::cout << " -i, --input Input video file or camera index (default: 0)\n";
    std::cout << " -o, --output Output video file (optional)\n";
    std::cout << " -m, --model YOLOv11 model path (default: models/yolov11n.onnx)\n";
    std::cout << " -f, --features Feature model path (default: models/resnet50_features.onnx)\n";
    std::cout << " -c, --confidence Confidence threshold (default: 0.3)\n";
    std::cout << " -l, --learning Learning rate (default: 0.02)\n";
...
Config parse_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if ((arg == "-g" || arg == "--gallery") && i + 1 < argc) {
            config.gallery_path = argv[++i];
        }
    }
    return config;
}
...
void save_performance_report(const std::vector<double>& processing_times,
    ...
    double avg_tracks = std::accumulate(track_counts.begin(), track_counts.end(), 0.0) / track_counts.size();
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (!config.gallery_path.empty()) {
            tracker.load_gallery(config.gallery_path);
        }
        ...
        if (!config.output.empty()) {
            ...
            writer.open(config.output, cv::VideoWriter::fourcc('M', 'P', '4', 'V'), fps, frame_size, true);
            if (!writer.isOpened()) {
        ...
        while (cap.read(frame)) {
            ...
            try {
                ...
                for (const auto& track : tracks) {
                    ...
                    std::string label = "ID:" + std::to_string(track.id) + " (" + std::to_string(static_cast<int>(track.confidence * 100)) + "%)";
                    cv::Size label_size = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, nullptr);
                    cv::Point label_pos(static_cast<int>(track.bbox.x), static_cast<int>(track.bbox.y) - 10);
                    if (label_pos.y < 0) label_pos.y = static_cast<int>(track.bbox.y + track.bbox.height + 20);
                    cv::rectangle(frame, cv::Point(label_pos.x, label_pos.y - label_size.height - 5),
                                  cv::Point(label_pos.x + label_size.width + 5, label_pos.y + 5), color, -1);
                    cv::putText(frame, label, label_pos, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0), 1);
                }
                if (config.benchmark || is_camera) {
                    ...
                    cv::putText(frame, perf_info, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
                }
                if (paused) {
            ...
            if (frame_count % 100 == 0) {
                if (!processing_times.empty()) {
                    double avg_time = std::accumulate(processing_times.begin(), processing_times.end(), 0.0) / processing_times.size();
                    std::cout << "Processed " << frame_count << " frames, avg: " << avg_time << "ms (" << static_cast<int>(1000.0 / avg_time) << " FPS)" << std::endl;
                }
            }
        }
        cap.release();
        ...
        if (!processing_times.empty()) {
            double avg_time = std::accumulate(processing_times.begin(), processing_times.end(), 0.0) / processing_times.size();
            double fps_calc = 1000.0 / avg_time;
            double avg_tracks = std::accumulate(track_counts.begin(), track_counts.end(), 0.0) / track_counts.size();
            ...
            std::cout << "Average FPS: " << fps_calc << std::endl;
